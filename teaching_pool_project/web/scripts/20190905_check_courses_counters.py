...
def run():
	...
	for course in all_courses:
		...
		# if there is no request for TA, then we default to the calculated one
			...
			if course.approvedNumberOfTAs == course.calculatedNumberOfTAs:
				# print("OK -> number of approved TAs equals the calculated one")
				pass
			else:
				print("\tKO -> number of approved TAs mismatch ({} vs {} calculated)".format(course.approvedNumberOfTAs, course.calculatedNumberOfTAs))
				if not course.approvedNumberOfTAs:
